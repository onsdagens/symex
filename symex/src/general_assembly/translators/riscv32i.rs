...
impl Translatable for Instruction {
    fn translate(&self) -> GAInstruction {
        match self.operation {
            Operation::LUI { rd, imm } => {
                let operations = vec![
                    ...
                    GAOperation::Move {
                        ...
                        source: Operand::Immidiate(DataWord::Word32(imm << 11)),
                    },
                ];
                GAInstruction {
                    instruction_size: 32,
            ...
            Operation::LBU { imm, rs1, rd } => {
                ...
                GAOperation::ZeroExtend { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rd), bits: 32 }
                ];
 ...
 GAOperation::ZeroExtend { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rd), bits: 32 }
            ...
            Operation::XORI { imm, rs1, rd } => {
                let operations = vec![
                    GAOperation::Xor { destination: riscv_reg_to_ga_operand(rd), operand1: riscv_reg_to_ga_operand(rs1), operand2: Operand::Immidiate(DataWord::Word32(imm as u32)) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::XOR { rs2, rs1, rd } => {
                let operations = vec![
                    GAOperation::Xor{destination: riscv_reg_to_ga_operand(rd), operand1: riscv_reg_to_ga_operand(rs1), operand2: riscv_reg_to_ga_operand(rs2)}
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::SLL { rs2, rs1, rd } => {
                let operations = vec![
                    GAOperation::Sl { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: riscv_reg_to_ga_operand(rs2) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::SRL { rs2, rs1, rd } => {
                let operations = vec![
                    GAOperation::Srl { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: riscv_reg_to_ga_operand(rs2) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::SRA { rs2, rs1, rd } => {
                let operations = vec![
                    GAOperation::Sra { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: riscv_reg_to_ga_operand(rs2) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::ORI { imm, rs1, rd } => {
                let operations = vec![
                    GAOperation::Or { destination: riscv_reg_to_ga_operand(rd), operand1: riscv_reg_to_ga_operand(rs1), operand2: Operand::Immidiate(DataWord::Word32(imm as u32)) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::ANDI { imm, rs1, rd} => {
                let operations = vec![
                    GAOperation::And { destination: riscv_reg_to_ga_operand(rd), operand1: riscv_reg_to_ga_operand(rs1), operand2: Operand::Immidiate(DataWord::Word32(imm as u32)) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::SLLI { shamt, rs1, rd } => {
                let operations = vec![
                    GAOperation::Sl { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: Operand::Immidiate(DataWord::Word8(shamt)) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::SRLI { shamt, rs1, rd } => {
                let operations = vec![GAOperation::Srl { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: Operand::Immidiate(DataWord::Word8(shamt)) }];
            ...
            Operation::SRAI { shamt, rs1, rd } => {
                let operations = vec![
                    GAOperation::Sra { destination: riscv_reg_to_ga_operand(rd), operand: riscv_reg_to_ga_operand(rs1), shift: Operand::Immidiate(DataWord::Word8(shamt)) }
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::CSRRW { csr, rs1, rd }  => { //these should not be nops maybe mov to
                //register CSR+1k or something
                let operations = vec![
                    GAOperation::Move { destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr)) },
                    GAOperation::Move {destination: Operand::Register(format!("{}", csr)), source: riscv_reg_to_ga_operand(rs1)}
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::CSRRS { csr, rs1, rd } => {
                let operations = vec![
                    GAOperation::Move { destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr)) },
                    GAOperation::Move { destination: Operand::Register("temp".to_string()), source: Operand::Register(format!("{}", csr))},
                    GAOperation::Or { destination: Operand::Register(format!("{}", csr)), operand1: riscv_reg_to_ga_operand(rs1), operand2: Operand::Register("temp".to_string()) },
                ];
            ...
            Operation::CSRRC { csr, rs1, rd } => {
                let operations = vec![
                    GAOperation::Move{destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr))},
                    GAOperation::Not { destination: Operand::Register("temp".to_string()), operand: riscv_reg_to_ga_operand(rs1) },
                    GAOperation::And { destination: Operand::Register(format!("{}", csr)), operand1: riscv_reg_to_ga_operand(rd), operand2: Operand::Register("temp".to_string()) },
                ];
            ...
            Operation::CSRRWI { csr, zimm, rd } => {
                let operations = vec![
                    GAOperation::Move { destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr)) },
                    GAOperation::Move {destination: Operand::Register(format!("{}", csr)), source: Operand::Immidiate(DataWord::Word8(zimm))}
                ];
                GAInstruction{
                    instruction_size: 32,
                    operations,
            ...
            Operation::CSRRSI { csr, zimm, rd } => {
                let operations = vec![
                    GAOperation::Move { destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr)) },
                    GAOperation::Move { destination: Operand::Register("temp".to_string()), source: Operand::Register(format!("{}", csr))},
                    GAOperation::Or { destination: Operand::Register(format!("{}", csr)), operand1: Operand::Immidiate(DataWord::Word8(zimm)), operand2: Operand::Register("temp".to_string()) },
                ];
            ...
            Operation::CSRRCI { csr, zimm, rd }  => {
                let operations = vec![
                    GAOperation::Move{destination: riscv_reg_to_ga_operand(rd), source: Operand::Register(format!("{}", csr))},
                    GAOperation::Not { destination: Operand::Register("temp".to_string()), operand: Operand::Immidiate(DataWord::Word8(zimm)) },
                    GAOperation::And { destination: Operand::Register(format!("{}", csr)), operand1: riscv_reg_to_ga_operand(rd), operand2: Operand::Register("temp".to_string()) },
                ];
            ...
            Operation::AND { rs2, rs1, rd } => {
                let operations = vec![
                    GAOperation::And { destination: riscv_reg_to_ga_operand(rd), operand1: riscv_reg_to_ga_operand(rs1), operand2: riscv_reg_to_ga_operand(rs2) },
                ];
       ...
       fn add_pc_hooks(hooks: &mut Vec<(Regex, crate::general_assembly::project::PCHook)>) {
        let symbolic_sized = |state: &mut GAState| {
            let value_ptr = state.get_register("A0".to_owned())?.unwrap();
            let size = 32;
            trace!(
           ...
           let lr = state.get_register("RA".to_owned())?.unwrap();
